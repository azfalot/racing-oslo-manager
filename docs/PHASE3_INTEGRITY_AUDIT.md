...
4. **Programmatic Parameter Selection:**
   ...
   - Shrinkage $K$: Programmatically selected from candidate evaluations ($k = 10$ with $\text{MAE} = 1.363$) and flagged with `status: "HEURISTIC_PRIOR"`.

5. **Symmetric Monte Carlo Season Simulation:**
   - `ACTUAL_BASELINE` runs symmetric aggregate forecasting for ALL clubs derived from `loadLiveClubBaselines()` (Racing aggregate PPM: $36.8$, Fermín: $49.0$).
...
| **Rest of Season Aggregate Baseline PPM** | `loadLiveClubBaselines()` | — | **36.8 pts** | — | — |
